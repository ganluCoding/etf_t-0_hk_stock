...
- 本机生成的最新 OHLC 回本账本：16 个已确认研究标的、1 万/1.5 万/3 万订单金额、5 个网格间距和 2 个费用情景共 480 行；可进入正式回测的行数为 0。该结果是缺少费用与可执行盘口证据时的预期 fail-closed 行为。
...
| 2026-08-02 | 现有数据策略证据复算 | 重跑 122 个多策略试验和 480 行回本账本；冻结唯一前向研究候选，实盘仍为 No-Go；记录回本门禁集成缺口 | [PR #42](https://github.com/ganluCoding/etf_t-0_hk_stock/pull/42)，Issue #41；160 项测试与 `ruff` 通过 |
